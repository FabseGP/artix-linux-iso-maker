...
# Updating pacman-config + backup of existing
  ...
  if [[ "$(pacman -Qs opendoas)" ]] && ! [[ "$(pacman -Qs run some commands as root)" ]]; then SU_COMMAND="doas"; 
...
# Assigning parameters
  ...
  if [[ -f "/etc/vconsole.conf" ]] && [[ -z "$KEYMAP" ]]; then KEYMAP_system="$(</etc/vconsole.conf)"; KEYMAP=${KEYMAP_system#*=}; fi
...
# Continues building the ISO with auto-cleanup
  ...
  echo
  echo "----------------------------------------------------------------"
  echo "------YOUR CUSTOM ISO CAN BE FOUND AT ISO/base------"
  echo "----------------------------------------------------------------"
  echo 
