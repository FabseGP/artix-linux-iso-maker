...
# Configurable parameters
  ...
  BUILD_DIR="" # Path to build the iso, e.g. /home/USERNAME/Downloads; the default path is /home/USERNAME/ARTIX_ISO
...
# Cleaning conflicting folders
  ...
  if [[ -d "$BUILD_DIR/BUILDISO" ]]; then
    if [[ -d "$BUILD_DIR/BUILDISO/buildiso/base/artix/bootfs" ]]; then sudo umount -l $BUILD_DIR/BUILDISO/buildiso/base/artix/bootfs; fi
    if [[ -d "$BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs" ]]; then sudo umount -l $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs; fi
    sudo rm -rf $BUILD_DIR/BUILDISO
  fi
...
# Copies configs and creates folders
  ...
  cp -rf artools $BUILD_DIR/.config 
  cp -rf artools-workspace $BUILD_DIR 
  mkdir $BUILD_DIR/{BUILDISO,ISO}
...
# Builds the filesystem and applies modifications
  ...
  sudo modprobe loop 
  buildiso -p base -x
  sudo sed -i 's/--noclear/--autologin root --noclear/' $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/etc/dinit.d/tty1
  sudo cp scripts/startup_choice.sh $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/etc/profile.d/startup_choice.sh
  sudo mkdir $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/{scripts,.nothing,.encrypt,.decrypt}
  sudo cp scripts/{startup.sh,startup_with_answerfile.sh,startup_wget_answerfile.sh,keymap.sh} $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/scripts
  sudo sed -i "3s/^/  KEYMAP=$KEYMAP_sorted\n/" $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/scripts/keymap.sh
  for answerfile in $ANSWERFILE_path_base $ANSWERFILE_path_minimal $ANSWERFILE_path_full; do
    if [[ "$answerfile" ]]; then
      mkdir $BUILD_DIR/.nothing$index
      date | sha512sum > $BUILD_DIR/.nothing$index/nothing$index.txt
      openssl enc -aes-256-cbc -md sha512 -a -pbkdf2 -iter 100000 -salt -in "$ANSWERFILE_path_base" -out $BUILD_DIR/.nothing$index/encrypt$index.txt -pass file:$BUILD_DIR/.nothing$index/nothing$index.txt
      sudo cp $BUILD_DIR/.nothing$index/{nothing$index.txt,encrypt$index.txt} $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/scripts
      rm -rf $BUILD_DIR/.nothing$index
      (( index++ )) || true
    else (( index++ )) || true; fi
  done
  sudo cp configs/wifi_backend.conf $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/etc/NetworkManager/conf.d/wifi_backend.conf
  sudo cp configs/{pacman_with_arch.conf,pacman_without_arch.conf} $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/
  sudo cp scripts/repositories.sh $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/
  artix-chroot $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/ /bin/bash -c "bash /repositories.sh"
  sudo rm -rf $BUILD_DIR/BUILDISO/buildiso/base/artix/rootfs/repositories.sh
